...
#       mode          Detection mode: 0 only not initialized, 1 only initialized, 2 all    
    ...
    cwefdaLoggerInfo("CWE4FDA find_all_variables : Scanning file " + str(pfile.name))
    ...
    try:
        ...
        cwefdaLoggerInfo("SPDB find_all_variables : File scanned (size %s) ", len(buffer))
        ...
        for p in StdTypes:    
            ...
            if (not results is None):
                for m in results:
                    var = m.group(1)
                    #logging.info("CWE4FDA find_all_variables : Single Variable with no init %s", str(var))
            ...
            if (not results is None):
                for m in results:
                    var = m.group(1)
                    #logging.info("CWE4FDA find_all_variables : Single Variable with init %s", str(var))
            ...
            if (not results is None):
                for m in results:
                    ...
                    for v in mv:
                        vi = v.group(1)
                        #logging.info("CWE4FDA find_all_variables : Variable in a multiple definition %s (from %s)", str(vi), str(mvar).strip())
                    ...
                    if (not m is None):
                        for im in m:
                            ...
                            if (var != funcName) and (not var in allVars):
                                #logging.info("CWE4FDA find_all_variables : Variable %s as parameter for function %s", str(var), funcName)
                                allVars.add(var)
                                varCount += 1
    ...
    cwefdaLoggerInfo("SPDB find_all_variables : Found %s variables", str(varCount))
...
#   Author :                 SCS
#   Last modification date : 27/4/2017
#   Description:             Update the log with stats for each CWE: throughput, duration, bytes processed
    global ExtraLogList   
    global ExtraLogBytes
    global ExtraLogMsecs
