...
#                                bk = Bookmark(pfile, current_line, p.start() + 1, current_line, p.end())
    ...
    if msecs == 0: 
        msecs = 1   
    logging.info("scan_file_SPDBviolation12_2_5 : END %s - Found %s violation ", str(pfile.name), str(nbViolation))    
...
#   Description: SPDBviolation14_1_1: Memory allocation with malloc should be checked with NULL 
    ...
    logging.info("14_1_1 : -------------------------------------------------------------------------")
    logging.info("14_1_1 : Starting scan_file_SPDBviolation14_1_1        > " + str(pfile.name))
...
#   Description: SPDBviolation14_1_5: Memory allocation with malloc should be checked with NULL 
    ...
    logging.info("14_1_5 : -------------------------------------------------------------------------")
    logging.info("14_1_5 : Starting scan_file_SPDBviolation14_1_5        > " + str(pfile.name))
 ...
 #                               logging.info("\n@@@@ "+getVar)
                                ...
                                if getVar.__contains__('='):
                                    ...
                                    if (not varNames is None):                                          
                                        ...
                                        boolExist = 1
                                        scan_Utilities.unConditionalCheck(line, pfile, current_line, p, f, varName, dtType) #, "SPDBviolation10_3_4")
                                else:                              
                                    logging.debug("scan_file_SPDBviolation10_3_4 :: Violation saved for getVar value---" + str(getVar))  
                                    boolExist = 1
                                    scan_Utilities.unConditionalCheck(line, pfile, current_line, p, f, getVar, dtType) #, "SPDBviolation10_3_4")
...
#    
    ...
    nBytes = 0
    logging.debug("pfile.name----" + str(pfile.name))
    logging.info("scan_file_SPDBviolation9_4_2 : -------------------------------------------------------------------------")
    logging.info("scan_file_SPDBviolation9_4_2 : Starting scan_file_scan_file_SPDBviolation9_4_2 > " + str(pfile.name))
        ...
        with open_source_file(pfile.get_path()) as f:
            ...
            for line in f:                  
                ...
                try:               
                    ...
                    if (not result is None):                                          
                        for p in result:   
                            logging.info("scan_file_scan_file_SPDBviolation9_4_2::Result is: [%s]  [%s]  [%s]", pfile, line, p.groups()) 
                            
#                            bk = Bookmark(pfile, current_line, p.start() + 1, current_line, p.end())
                            bk = Bookmark(pfile, current_line, 1, current_line, -1)
                            pfile.save_violation('SPDB_CustomMetrics_C.SPDBviolation9_4_2', bk)
                                    
                except Exception as e:
                    logging.error("scan_file_SPDBviolation9_4_2 : Error: %s, at line ", str(e), current_line)
        ...
        logging.error("scan_file_SPDBviolation9_4_2 : File not found > " + str(pfile.get_path()))
    except Exception as e:
    ...
    if msecs == 0: 
        msecs = 1   
    logging.info("scan_file_SPDBviolation9_4_2 : END %s - Found %s violation ", str(pfile.name), str(nbViolation))    
