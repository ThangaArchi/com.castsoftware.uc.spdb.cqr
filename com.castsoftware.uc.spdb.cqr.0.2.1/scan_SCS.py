...
#   Description: scan_file_SPDBviolation14_1_3
#   NOTE
#   As the rule is widely general and largely semantic, it is tailored only on specific patterns.
#   It simply finds all function calls which are not assigned to vars.
#    
    ...
    logging.info("scan_file_SPDBviolation14_1_3 : -------------------------------------------------------------------------")
    logging.info("scan_file_SPDBviolation14_1_3 : Starting scan_file_SPDBviolation14_1_3 > " + str(pfile.name))
...
#                logging.error("Current line %s", line) 
    ...
    if msecs == 0: 
        msecs = 1   
    logging.info("scan_file_SPDBviolation14_1_3 : END scan_file %s - Found %s violation ", str(pfile.name), str(nbViolation))    
...
#        if p.group(1).find("NULL") != -1:
            logging.error("NO-VIOLATION MALLOC is validated with NULL - lines[1] %s", line)
        else:
#            bk = Bookmark(pfile, current_line, p.start() + 1, current_line, p.end())
            bk = Bookmark(pfile, current_line, 1, current_line, -1)
